...
public class Can_be_related_to
{
    ...
    public void matching_month()
        => new DateTime(2017, 06, 11).InMonth(Month.June).Should().BeTrue();

    [Test]
    public void none_matching_month()
       => new DateTime(2017, 06, 11).InMonth(Month.February).Should().BeFalse();

    [Test]
    public void matching_year()
        => new DateTime(2017, 06, 11).InYear(2017.CE()).Should().BeTrue();

    [Test]
    public void none_matching_year()
       => new DateTime(2017, 06, 11).InYear(2018.CE()).Should().BeFalse();
}
...
public class Can_not_be_related_to
{
    ...
    public void month_empty()
        => new DateTime(2017, 06, 11).InMonth(Month.Empty).Should().BeFalse();

    [Test]
    public void month_unknown()
       => new DateTime(2017, 06, 11).InMonth(Month.Unknown).Should().BeFalse();

    [Test]
    public void year_empty()
        => new DateTime(2017, 06, 11).InYear(Year.Empty).Should().BeFalse();

    [Test]
    public void year_unknown()
       => new DateTime(2017, 06, 11).InYear(Year.Unknown).Should().BeFalse();
}
...
public class Can_be_adjusted_with
{
    ...
    public void Date_span_with_days_first()
        => new DateTime(2017, 06, 11).Add(new DateSpan(2, 20), true).Should().Be(new DateTime(2017, 09, 01));
