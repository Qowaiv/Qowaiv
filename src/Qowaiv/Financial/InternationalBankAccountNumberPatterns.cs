...
namespace Qowaiv.Financial
{
    public partial struct InternationalBankAccountNumber
    {
        ...
        {   
            ...
            // Belarus, Length: 28, BBAN: 4c, 20n, Fields: BYkk bbbb aaaa cccc cccc cccc cccc (b = National bank code, a = Balance account number, c = Account number)
            { Country.BY, new Regex(@"^BY[0-9]{2}[A-Z0-9]{24}$", RegexOptions.Compiled) },
            ...
            // East Timor, Length: 23, BBAN: 2n, 17c 2n, Fields: TLkk bbbc cccc cccc cccc cxx (k = IBAN check digits (always = "38"), b = Bank identifier, c = Account number, x = National check digits)
            { Country.TL, new Regex(@"^TL38[0-9A-Z]{17}[0-9]{2}$", RegexOptions.Compiled) },
            ...
            // North Macedonia, Length: 19, BBAN: 3n,10c,2n, Fields: MKkk bbbc cccc cccc cxx (k = IBAN check digits (always = "07"), b = National bank code, c = Account number, x = National check digits)
            ...
            // Palestinian territories, Length: 29, BBAN: 4c,21n, Fields: PSkk bbbb xxxx xxxx xccc cccc cccc c (b = National bank code, c = Account number, x = Not specified)
            ...
            // Vatican City, Length: 22, BBAN: 18n, Fields: VAkk bbbc cccc cccc cccc cc (b = National bank code, c = Account number)
            { Country.VA, new Regex(@"^VA[0-9]{20}$", RegexOptions.Compiled) },
            ...
            #region Nordea has catalogued IBANs for countries listed below.
            ...
            // Angola, Length: 25, BBAN: 21n, Fields: AOkk nnnn nnnn nnnn nnnn nnnn n
            { Country.AO, new Regex(@"^AO[0-9]{23}$", RegexOptions.Compiled) },
            
            // Benin, Length: 28, BBAN: 1a, 23n, Fields: BJkk annn nnnn nnnn nnnn nnnn nnnn
            { Country.BJ, new Regex(@"^BJ[0-9]{2}[A-Z][0-9]{23}$", RegexOptions.Compiled) },
            
            // Burkina Faso, Length: 27, BBAN: 23n, Fields: BFkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.BF, new Regex(@"^BF[0-9]{25}$", RegexOptions.Compiled) },
            
            // Burundi, Length: 16, BBAN: 12n, Fields: BIkk nnnn nnnn nnnn
            { Country.BI, new Regex(@"^BI[0-9]{14}$", RegexOptions.Compiled) },
            
            // Cameroon, Length: 27, BBAN: 23n, Fields: CMkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.CM, new Regex(@"^CM[0-9]{25}$", RegexOptions.Compiled) },
            
            // Cape Verde, Length: 25, BBAN: 21n, Fields: CVkk nnnn nnnn nnnn nnnn nnnn n
            { Country.CV, new Regex(@"^CV[0-9]{23}$", RegexOptions.Compiled) },
            
            // Iran, Length: 26, BBAN: 22n, Fields: IRkk nnnn nnnn nnnn nnnn nnnn nn
            { Country.IR, new Regex(@"^IR[0-9]{24}$", RegexOptions.Compiled) },

            // Ivory Coast, Length: 28, BBAN: 1a, 23n, Fields: CIkk annn nnnn nnnn nnnn nnnn nnnn
            { Country.CI, new Regex(@"^CI[0-9]{2}[A-Z]{23}$", RegexOptions.Compiled) },
  
            // Madagascar, Length: 27, BBAN: 23n, Fields: MGkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.MG, new Regex(@"^MG[0-9]{25}$", RegexOptions.Compiled) },
            
            // Mali, Length: 28, BBAN: 1a, 23n, Fields: MLkk annn nnnn nnnn nnnn nnnn nnnn
            { Country.ML, new Regex(@"^ML[0-9]{2}[A-Z][0-9]{23}$", RegexOptions.Compiled) },
            
            // Mozambique, Length: 25, BBAN: 21n, Fields: MZkk nnnn nnnn nnnn nnnn nnnn n
            { Country.MZ, new Regex(@"^MZ[0-9]{23}$", RegexOptions.Compiled) },

            // Senegal, Length: 28, BBAN: 1a, 23n, Fields: SNkk annn nnnn nnnn nnnn nnnn nnnn
            ...
            #endregion In the process of introducing the IBAN retrieved from SWIFT partner website

            #region In the process of introducing the IBAN retrieved from SWIFT partner website 

            // Comoros, Length: 27, BBAN: 23n, Fields: KMkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.KM, new Regex(@"^KM[0-9]{25}$", RegexOptions.Compiled) },

            // Chad, Length: 27, BBAN: 23n, Fields: KMkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.TD, new Regex(@"^TD[0-9]{25}$", RegexOptions.Compiled) },

            // Congo, Length: 27, BBAN: 23n, Fields: CGkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.CG, new Regex(@"^CG[0-9]{25}$", RegexOptions.Compiled) },

            // Egypt, Length: 27, BBAN: 23n, Fields: EGkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.EG, new Regex(@"^EG[0-9]{25}$", RegexOptions.Compiled) },

            // Gabon, Length: 27, BBAN: 23n, Fields: GAkk nnnn nnnn nnnn nnnn nnnn nnn
            { Country.GA, new Regex(@"^GA[0-9]{25}$", RegexOptions.Compiled) },

            // Honduras, Length: 28, BBAN: 4a, 20n, Fields: HNkk aaaa nnnn nnnn nnnn nnnn nnnn
            { Country.HN, new Regex(@"^HN[0-9]{2}[A-Z]{4}[0-9]{20}$", RegexOptions.Compiled) },

            // Morocco, Length: 28, BBAN: 24n, Fields: GAkk nnnn nnnn nnnn nnnn nnnn nnnn
            { Country.MA, new Regex(@"^MA[0-9]{26}$", RegexOptions.Compiled) },

            // Nicaragua, Length: 32, BBAN: 4a, 24n, Fields: NIkk aaaa nnnn nnnn nnnn nnnn nnnn nnnn
            { Country.NI, new Regex(@"^NI[0-9]{2}[A-Z]{4}[0-9]{24}$", RegexOptions.Compiled) },

            // Niger, Length: 28, BBAN: 2a, 22n, Fields: NEkk aann nnnn nnnn nnnn nnnn nnnn
            { Country.NE, new Regex(@"^NE[0-9]{2}[A-Z]{2}[0-9]{22}$", RegexOptions.Compiled) },

            // Togo, Length: 28, BBAN: 2a, 22n, Fields: TGkk aann nnnn nnnn nnnn nnnn nnnn
            { Country.TG, new Regex(@"^TG[0-9]{2}[A-Z]{2}[0-9]{22}$", RegexOptions.Compiled) },
