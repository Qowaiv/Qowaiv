...
#endif
{
    ...
    private void AppendNumerator(StringBuilder sb, long remainder, string format, IFormatProvider? formatProvider)
    {
        if (format == "super")
        {
            ...
            var super = remainder.Abs().ToString(CultureInfo.InvariantCulture).Select(ch => Formatting.SuperScript[ASCII.Digit(ch)]).ToArray();
            sb.Append(super);
        }
        else
        {
    ...
    private void AppendDenominator(StringBuilder sb, string format, IFormatProvider? formatProvider)
    {
        ...
        {
            // use invariant as we want to convert to superscript.
            var super = Denominator.ToString(CultureInfo.InvariantCulture).Select(ch => Formatting.SubScript[ASCII.Digit(ch)]).ToArray();
            sb.Append(super);
        }
        else
        {
