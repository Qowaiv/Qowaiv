...
namespace Qowaiv.UnitTests.TestTools
{
    ...
    public static class TypeConverterAssert
    {
        ...
        [DebuggerStepThrough]
        public static void ConvertToStringEquals<T>(string expected, T input) => ConvertToEquals(expected, input);

        /// <summary>Asserts that the TypeConverter converts the input value to the expected string.</summary>
        [DebuggerStepThrough]
        public static void ConvertToEquals<TInput, TOutput>(TOutput expected, TInput input)
        {
            var converter = TypeDescriptor.GetConverter(typeof(TInput));
            var actual = converter.ConvertTo(input, typeof(TOutput));
